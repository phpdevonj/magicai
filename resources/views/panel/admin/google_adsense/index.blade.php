            ...
            <div class="card border-0">
                <div class="card-header">
                    ...
                    <span class="inline-block relative">
                        ...
                        <path d="M6.66732 0.333313C10.3473 0.333313 13.334 3.31998 13.334 6.99998C13.334 10.68 10.3473 13.6666 6.66732 13.6666C2.98732 13.6666 0.000652313 10.68 0.000652313 6.99998C0.000652313 3.31998 2.98732 0.333313 6.66732 0.333313ZM6.00065 10.3333H7.33398V6.33331H6.00065V10.3333ZM6.00065 4.99998H7.33398V3.66665H6.00065V4.99998Z"></path>
                        </svg>
                        </span>
                        <div class="absolute bottom-[calc(100%+0.75rem)] !start-0 min-w-[230px] !p-3 bg-[#fff] bg-opacity-5 text-[13px] font-medium tex-heading rounded-md backdrop-blur-md backdrop-saturate-150 duration-300 border border-solid !border-black !border-opacity-10 !opacity-0 -translate-y-2 transition-all shadow-[0_4px_15px_rgba(0,0,0,0.05)] pointer-events-none peer-hover:!opacity-100 peer-hover:translate-y-0">Activate header section to view ads</div>
                    </span>
                </div>
                <div class="card-body">
                    <table class="table dataTable no-footer dtr-inline w-100" id="adsTable">
                        <thead>
                        <tr>
                            <th>{{__('Adsense Type')}}</button></th>
                            <th>{{__('Status')}}</button></th>
                            <th>{{__('Updated on')}}</button></th>
                            <th class="!text-end">{{__('Actions')}}</th>
                        </tr>
                        </thead>
                        <tbody class="table-tbody align-middle text-heading">
                        @foreach($googleAdsense as $adsense)
                            <tr>
                                <td class="sort-name">{{ $adsense->type }}</td>
                                <td class="sort-group">
                                    @if($adsense->status)
                                        <span class="badge bg-success text-success">Activated</span>
                                    @else
                                        <span class="badge bg-danger text-danger">Deactivated</span>
                                    @endif
                                </td>
                                <td class="sort-remaining-words">{{ date('d M Y H:i A', strtotime($adsense->updated_at ?? $adsense->created_at)) }}</td>
                                <td class="!text-end whitespace-nowrap">
                                    <a href="{{ route('dashboard.admin.adsense.edit', $adsense->id) }}" class="btn w-[36px] h-[36px] p-0 border hover:bg-[var(--tblr-primary)] hover:text-white" title="{{__('Edit')}}">
